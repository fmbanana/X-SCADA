...
(function($) {// jscs:ignore requireNamedUnassignedFunctions
	...
	function getPropertyRowHtml(pgId, name, value, meta, postCreateInitFuncs, getValueFuncs, options, el) {
		...
		// If boolean create checkbox
		  ...
		  //If font and we have the font picker use it
		else if (type === 'font' && typeof $.fn.spectrum === 'function') {
			valueHTML = '<input type="text" id="' + elemId + '" />';		

			if(postCreateInitFuncs){
				postCreateInitFuncs.push(initFontPicker(elemId, value, meta.options, name, changedCallback, el));
			}
	...
	function initFontPicker(id, initfont, options, name, changedCallback, el) 
	{
		if (!id) {
			return null;
		}

		return function onFontPickerInit() {
			var $elem = $('#' + id);

			// apply initfont for preview current font
			applyPreviewFont($elem, initfont);

			// init font picker with input options
			$elem.fontpicker(options);

			// assigned callback when have a changing of selection
			$elem.on('change', function changed() {
				changedCallback(el, name, this.value);
			 });
		};
	}

	function applyPreviewFont(element, fontSpec) {
